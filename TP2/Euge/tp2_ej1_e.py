# -*- coding: utf-8 -*-
"""
Created on Tue Sep 12 13:58:28 2017
...
# Permutaciones (solo porque no falla)
    ...
    #e) Repetir los puntos b) y d) para los valores normalizados.
     # Diferencias por bandas Normalizadas
    #Alpha: 
    AlphaP = stats.zscore(np.array(df2.Alpha[df2.Grupo == 'P']))
    AlphaS = stats.zscore(np.array(df2.Alpha[df2.Grupo == 'S']))
    pperm1 = exact_mc_perm_test2(AlphaP,AlphaS, 1000) 
    print('NAlpha P vs S: p=' + str(pperm1)+ ' (permutaciones).')   
    
    #Beta: 
    BetaP = stats.zscore(np.array(df2.Beta[df2.Grupo == 'P']))
    BetaS = stats.zscore(np.array(df2.Beta[df2.Grupo == 'S']))
    pperm2 = exact_mc_perm_test2(BetaP,BetaS, 1000) 
    print('NBeta P vs S: p=' + str(pperm2)+ ' (permutaciones).')  
    
    #Gamma: 
    GammaP = stats.zscore(np.array(df2.Gamma[df2.Grupo == 'P']))
    GammaS = stats.zscore(np.array(df2.Gamma[df2.Grupo == 'S']))
    pperm3 = exact_mc_perm_test2(GammaP,GammaS, 1000) 
    print('NGamma P vs S: p=' + str(pperm3)+ ' (permutaciones).')  
    
    #Delta: 
    DeltaP = stats.zscore(np.array(df2.Delta[df2.Grupo == 'P']))
    DeltaS = stats.zscore(np.array(df2.Delta[df2.Grupo == 'S']))
    pperm4 = exact_mc_perm_test2(DeltaP,DeltaS, 1000) 
    print('NDelta P vs S: p=' + str(pperm4)+ ' (permutaciones).')  
    
    #Thera: 
    TheraP = stats.zscore(np.array(df2.Thera[df2.Grupo == 'P']))
    TheraS = stats.zscore(np.array(df2.Thera[df2.Grupo == 'S']))
    pperm5 = exact_mc_perm_test2(AlphaP,AlphaS, 1000) 
    print('NThera P vs S: p=' + str(pperm5)+ ' (permutaciones).')    
